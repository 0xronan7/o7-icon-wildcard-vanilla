...
export function o7Icon() {
    return {
        name: 'o7-icon',
        enforce: 'post',
        async transform(input, id) {
            if (id.includes('o7-icon/dist') || id.includes('node_modules'))
                return;
            let ast;
            try {
            ...
            walk(ast, null, {
                ImportDeclaration(node) {
                    if (typeof node.source.value !== 'string')
                        return;
                    if (!node.source.value.startsWith('@o7/icon'))
                        return;
            ...
            for (const node of imports) {
                ...
                for (const spec of node.specifiers) {
                    ...
                    if (spec.imported.type !== 'Identifier')
                        continue;
                    const source = node.source.value === '@o7/icon'
